...
public:
  ...
  void process(const std::string &input, const std::string &path) override {
    ...
    if (strptime(input.c_str(), "[%H:%M:%S] *** ", &time) != nullptr) { // Event block
      if (std::size_t open_user = input.find("*** Joins: ") != std::string::npos) { // Join block
        open_user += 11; // Increase by length of the literal string above "*** Joins: "
        std::size_t end_user = input.find_first_of(' ', open_user);

        if (end_user != std::string::npos) publishJoin(JoinEvent(std::move(time), input.substr(open_user, end_user - open_user)));

      } else if (std::size_t open_user = input.find(" is now known as ") != std::string::npos) { // Name change block
        ...
        std::size_t open_user = input.find("*** ") + 4; // Increase by 4 so we're at the start of the username
        KickEvent kick;
      ...
      } else if (bool part = input.find("*** Parts: ") != std::string::npos || input.find("*** Quits: ") != std::string::npos) {
        // Here we're going to check for quits and parts
        open_user = input.find("*** ");
        open_user = input.find(": ", open_user) + 2; // Increase by 2 to be just after the space
        std::size_t end_user = input.find_first_of(' ', open_user);
      ...
      return;
    } else if (strptime(input.c_str(), "[%H:%M:%S] * ", &time) != nullptr) { // Action block
      Message output;
      output.action(true)
            .time(std::move(time));
      ...
      open_user += 4; // Increase it by 4 so we're at the first character of the username
      std::size_t end_user = input.find_first_of(' ', open_user);

      output.author(input.substr(open_user, end_user - open_user))
            .message(input.substr(end_user + 2)); // + 2 to skip the > of the username and the space just after it

      publishMessage(std::move(output));

      return;
    } else if (strptime(input.c_str(), "[%H:%M:%S]", &time) == nullptr) throw std::runtime_error("Failed to parse time");

    std::size_t open_user = input.find_first_of('<');
    std::size_t end_user = input.find_first_of('>');
    if (open_user == std::string::npos || end_user == std::string::npos) throw std::runtime_error("Uhm what is this?");
    ...
    output.author(input.substr(open_user + 1, end_user - open_user - 1)) // +1 to the open to skip the < and - 1 to the end to skip the >
          ...
          .time(std::move(time)); // + 2 to skip the > of the username and the space just after it
